...
const CJK_TEXT = /[一-鿿]/;
function extractAsciiEntities(text) {
...
function extractCjkEntities(text) {
...
export class EntityGraph {
    adjacency = new Map();
    messageEntities = [];
    normalizedRank = new Map();
    entityDocumentFrequency = new Map();
    totalMessageCount = 0;
    build(messages, options = {}) {
        ...
        const cjkMessageCount = rawEntities.filter((entities) => entities.some((entity) => CJK_TEXT.test(entity))).length;
        ...
        const isPruned = (entity) => pruningActive && CJK_TEXT.test(entity)
        ...
        const addEdge = (from, to) => {
            if (from === to)
                return;
            const row = edges.get(from) ?? new Map();
    ...
    pageRank(options = {}) {
        ...
        const outDegrees = new Map(nodes.map((node) => [node, Math.max(1, this.adjacency.get(node)?.size ?? 0)]));
        ...
        this.normalizedRank = new Map(nodes.map((node) => [node, max > 0 ? (rank.get(node) ?? 0) / max : 0]));
        return new Map(this.normalizedRank);
    }
    /**
    ...
    entitiesForMessage(messageIndex) {
