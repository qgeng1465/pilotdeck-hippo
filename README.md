...
2. **真正站得住的结论在 B 列：6/10 → 10/10，两种措辞下都成立，是这张表唯一可对外的结论**。压缩后的上下文里，**当前话题**的精确事实被逐字保留，而上游只能从 stub 摘要里拿到 6/10。这与 §4.1 的主效应同源：**收益来自对"当前任务"的保真，而不是对旧上下文的记忆。**（decorrelated 下 tokens 也略降，因为不再为 A 段花预算。）
3. **两条诚实边界，都标出来而不是绕过去**：
   - 这个 harness 用固定文本 stub 摘要器，**摘要本身按构造不可能携带事实**，所以 `summary-only` 两臂都是 0，逐字列就是全部结论。这张表只回答"逐字保留块里有没有事实"，不回答"真摘要器下用户能不能答对问题"——后者要靠 §4.3 的真 LLM 闭环，而 §4.3 **没有覆盖话题切换**。
