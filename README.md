...
这一节是给我们自己泼冷水的，也方便评委按起分点逐条对质。下面每一项都能在对应各节引用的 `benchmarks/results/*.json` 里对账，这里重点写我们**不敢夸**的地方。赛道三的起分点是机制理解、改进深度、技术有效性、验证方式、代码质量，逐条摊开。

**机制理解**：改动没有去动上游压缩流水线，只是给 `CompactionEngine` 加了一个可选入口 `scorePolicy`。真正见理解的是对上游数据流的判断：把消息从待摘要桶里捞走会破坏 tool_call/tool_result 配对，所以保留块回流后先按原索引重排、再走 `stripUnpairedToolCalls/Results`。核心洞察是"选择**原文保留**、把这部分内容移出有损路径"，而不是"换个更好的摘要器"——这两者的差别在数据模型层面，不在 prompt。

**改进深度**：最深的一处是引擎层的原文保留机制本身——硬预算（20% tail、下限 256、封顶）、输出逐字而非转述、失败即退回纯上游，三条合起来才是能落地的工程决策。**打分部分我们承认是常规**：语义匹配（BM25/本地 embedding）+ 位置衰减，本质是相似度选句；最花哨的 IDF-PageRank 实体图谱，**生产默认权重是 0**——它在留出集上一对一 7 胜 33 平 0 负、四格全不显著（§4.5），结论是没带来可测量收益，所以保留为可配置项、不硬占权重。评委若问"这是不是默认死代码"，如实回答：功能完好、`wRank` 可开，但当前发布**不以它**作为得分依据——这是取舍，不是掩盖。

**技术有效性**：每个旗舰数字都能在 `benchmarks/results/*.json` 对到具体行，聚合口径（mean/median、用的哪个 seed 集）在 §4.8 表里逐条写清，未发现的聚合差异就不重复了。两处最容易被误读的口径，先自己讲：

- **17.9/20 是假摘要器下的"逐字保留召回代理"，不是"回答正确率"。** 上游的 0 是结构性对照（假摘要器不吐 FACT 文本）设计出来的。它落到真实任务（QA）上站得住的结论是：**中文、预算吃紧时优势稳定**（§4.3 轮 B 中文两格 62.5%/56.3% vs 0%/31.3%；§4.7 紧预算下当前话题 10/16 → 16/16）。**预算宽裕或英文时，无差异甚至饱和**——预算够宽时"保住事实"上游自己就能做到。我们宁可把优势范围讲窄，也不让 17.9 被误读成"回答对了 90%"。
- **真 LLM 英文格随预算变化**：轮 A 上游英文 N=160 只有 12.5%，轮 B 预算加宽后上游回到 100%（"长度与得分同向变化是测得的、因果关系仍是推断"已在 §4.3 写明）。对外只报方向和代价方向，不报"提升 X 个点"。

**验证方式**：相对扎实的是留出集 seed（20260921+ 从未参与任何参数选择）、配对双侧符号检验（并明确 p=0.002 表示"10 战全胜"、不是效应大）、逐消息 precision（用引擎自曝的 `retainedMessageTexts`，不反推命中、无重建伪影）、真 LLM 闭环带重复测量，外加一段主动的负面——旧话题保真被证明是句式模板共享造成的假象（§4.6）。**会挨批评的三条，我们先自己说**：① judge 与 summarizer 是同一模型（DeepSeek-V4-Flash），存在自我评价偏差；② 真 LLM 每格只有 16 题、2 个 seed，方向性结论下得谨慎；③ 主基准用合成、模板化事实，离真实长对话的噪声与主题漂移还有距离。这三条我们没有更优的缓解办法，也不打算装成没有。

**代码质量**：自认为最高的是三处——失败即退让（打分异常回退纯上游、embedding 加载超时回退 BM25）；保留块上的工具配对完整性处理；以及一批**专门设计成"不修就红"**的回归测试，例如 idle-timeout 测试故意让进程只剩这一个 pending work，`unref()` 没修就必挂；tokenizer 等价测试覆盖 900+ 组输入、0 差异。诚实的说明：引擎层与上游逐字节对齐的守护由 golden baseline + CI 里的 `benchmark:no-regression` 承担，**本地 `pnpm test` 跑不到这一层**（详见 §5）。

评审很可能追问，把答案先摆出来：

| 评委可能问 | 我们的答案 |
|---|---|
| 17.9 是"回答正确率"吗？ | 不是，是假摘要器下的逐字保留召回代理；真实 QA 优势在"中文 × 预算吃紧"这一窄区间 |
| IDF-PageRank 权重 0，是不是摆设？ | 留出集上一对一审过（7 胜 33 平 0 负、四格不显著）才设为 0；保留为可配置项，发布不以它作得分依据 |
| 为什么中文稳、英文不稳？ | v4-flash 中文摘要不复述数值事实、英文预算宽裕时能写进摘要——这正是"保留块在摘要顶不住预算时救场"的机制主线 |
| 上游是 0，是不是你们放水？ | 假摘要器结构性对照；真 LLM 下英文/宽预算无差异已在 §4.3、§4.7 如实写出 |
| judge 与摘要器同模型，会不会自卖自夸？ | 会，§4.3 已声明为已知偏差，结论只给反向、不报"提升 X 分" |
