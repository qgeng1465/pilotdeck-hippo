...
export class EbbinghausPageRankPolicy {
    options;
    id = "ebbinghaus-pagerank";
    constructor(options = {}) {
    ...
    async pickRetained(input) {
        if (input.candidates.length === 0 || input.retentionBudgetTokens <= 0)
            return [];
        ...
        if (isCarryOverEnabled()) {
            ...
            for (const entry of scored) {
                if (!isHippoRetained(entry.message))
                    continue;
                const tokens = eligible(entry.message);
                if (tokens === undefined)
                    continue;
                if (allowanceUsed + tokens > allowance)
                    continue;
                if (usedTokens + tokens > input.retentionBudgetTokens)
                    continue;
        ...
        for (const entry of scored) {
            if (retainedSet.has(entry.message))
                continue;
            const tokens = eligible(entry.message);
            if (tokens === undefined)
                continue;
    ...
    async score(candidates, queryHint) {
...
export function buildEbbinghausPageRankPolicy(options = {}) {
 ...
 */
export function resolveRetentionScorePolicy(compaction) {
