...
export type PilotAgentConfig = {
  ...
  /**
   * Compaction tuning. `retention` selects the pre-compaction retention
   * policy applied by the local gateway: "hippo" keeps high-value messages
   * verbatim via the Ebbinghaus/IDF-PageRank score (default in this fork),
   * "off" restores plain upstream compaction.
   */
  compaction?: { retention?: "hippo" | "off" };
};
