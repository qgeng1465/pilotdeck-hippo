...
前几节的转录稿都是"一路问到底"。真实使用更常见的是**中途换话题**：前半段在聊 A，尾部开始问 B，压缩时只有 B 的请求在手。此时 A 段消息**拿不到与其话题相关的 query 相关性**，只能靠残余的相似度去争取预算的剩余部分。这正好是衡量"换话题后旧上下文还在不在"的场景——而结论（见下）是**这一列被高估了**，原因值得完整摊开。

协议：10 个留出集种子，每个话题 10 条事实（共 20 条），先 A 后 B，压缩时 pending 请求是 B 的。脚本 `pnpm benchmark:topic-switch`，本节表格出自 `benchmarks/results/topic-switch-2026-09-11T11-19-16-507Z.json`；机制诊断脚本 `pnpm benchmark:topic-diagnosis`，出自 `benchmarks/results/topic-diagnosis-2026-09-11T11-44-03-797Z.json`。
...
1. **上游的 A 列必然是 0，别读成"上游把旧话题弄丢了"**：上游压根没有逐字保留机制，压缩后只剩一段固定文本 stub 摘要，而 stub 按构造不含任何事实（`summary-only` 两臂都是 0）。所以 "0 vs 5.9" 这个对比里，一半是"有没有保留机制"，另一半才是"保留得准不准"——后者正是第 2 条要打折的部分。
2. **Hippo 让旧话题留下来了一部分**（N=160 时 5.9/10），但**机制不是"记住了旧话题"——这一列经诊断后发现被高估了**。诊断脚本 `pnpm benchmark:topic-diagnosis`（JSON 随仓库提交），三条证据：
   - **单目标消融**（同一批转录稿，只留一个权重项）：`sim only` 单独就复现 **5.0/10**，而 `time only` 与 `rank only` 都保留 **0/10**。既不是时近度，更不是实体图——默认 `wRank = 0`，实体图本来就不参与打分。
   - **存活事实的位置**：在 A 段中的位置均值 0.58（0=最早、1=最新），且有 10/59 落在 0.00。**没有"越近越留"的倾向**，进一步排除时近度。
   - **直接量 BM25**：A 段事实对 B 请求的相似度 **0.820**，远高于 A 段填充消息的 **0.240**；把同样的事实改写成不含共享字段标签的散文后降到 **0.370**——对填充消息的那份优势，约 **78% 来自共享句式**。

   **结论（要对外说清楚）**：A 段被留下，主要是因为**本 harness 的 A、B 两类事实共用同一套字段模板**（`<topic>-FACTn: cohort=… n=371 gene=… freq=… stat=…`），于是"关于 B 的提问"在词面上天然贴近 A 段事实。**这不是"压缩保住了旧话题"的证据**，A 列因此被高估，我们不把它当作能力声明。要得到可信的旧话题保留率，得先把 harness 改成"事实的形式不泄露话题"再重测——**这是已知的实验缺口，见第 3 条**，我们不揣着一个好看的数字蒙混过去。新话题 B 从 6/10 提到 10/10 这一列没有该问题（B 与请求本就近）。
3. **两条诚实边界，都标出来而不是绕过去**：
   - 这个 harness 用固定文本 stub 摘要器，**摘要本身按构造不可能携带事实**，所以 `summary-only` 两臂都是 0，逐字列就是全部结论。这张表只回答"逐字保留块里有没有旧话题事实"，不回答"真摘要器下用户能不能答对旧话题问题"——后者要靠 §4.3 的真 LLM 闭环，而 §4.3 **没有覆盖话题切换**。
   - 由第 2 条的诊断可知，**A 列在"事实形式不泄露话题"之前不可作为能力声明**。修法是让 A/B 事实的句式与字段名互不重叠（或改用散文体），使话题成为唯一的区分因素，然后重跑本表；在那之前本表的 A 列只应作为**harness 内部一致性检查**使用。这是本项目**最明确的一个未闭合缺口**。
...
- 测试数量声明需可复核：**根套件**（`pnpm test`，`tests/**/*.spec.ts` 经 `dist/` 运行）2026-09-11 实测 **514 项 = 512 通过 / 0 失败 / 0 cancelled / 2 skipped**（40.0 s，退出码 0）。UI 侧是**独立**的 vitest 套件（约 900 例，状态见上一条），两者不合并计数——本文出现的 "514" 一律只指根套件。此前根套件为 481 通过 + 2 cancelled —— 那 2 项 cancelled 不是"满载并发的计时抖动"（早前版本如此解释过，此处撤回），而是 §5.1 缺陷三 的真实缺陷，修掉后取消项归零。
