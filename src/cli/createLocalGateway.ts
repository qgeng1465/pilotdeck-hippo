  ...
  private async prepareSessionRuntime(context: GatewaySessionContext) {
    ...
    const extendDependencies = (storage: ReturnType<typeof createAgentProjectSessionStorage>) => {
      ...
      const compactionEngine = new CompactionEngine({
        ...
        // Hippo selective retention (this fork). On by default; disable with
        // `agent.compaction.retention: off` in ~/.pilotdeck/pilotdeck.yaml.
        scorePolicy: resolveRetentionScorePolicy(runtime.snapshot.config.agent.compaction),
