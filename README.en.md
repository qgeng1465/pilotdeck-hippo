...
- **`ui/`'s `tsc --noEmit` exits 0.** The fork's starting point was 101 files / 511 errors, 501 of them from one root cause: two `@types/react` majors coexist in the graph (`ui`'s own 18.3.29 and the root-hoisted 19.2.15), and `lucide-react` declares no `@types/react` peer, so its `.d.ts` walked up to 19 and every icon failed the JSX component check. The fix pins `react` / `react/*` to ui's own types via `paths` in `ui/tsconfig.json` (typecheck-only; it does not touch `vite build`'s runtime alias, and needs no reinstall). The remaining 10 were genuine type gaps, each closed by typing the real thing (`subagentState.isFailed`, `NormalizedMessage.userHint`, `RefObject<HTMLInputElement>`, and so on) — no `@ts-ignore`, no `as any`. `vite build` exits 0 (40.4 s).
- **Retention-badge test boundary (stated honestly)**: gateway-side "retention ships with agent_status and the upstream path carries no such key" is covered by root suite `tests/context/retention-reporting.spec.ts`; the `compactMetadata → badge data` mapping (including malformed inputs not throwing) by 14 UI cases in `ui/src/components/chat/hooks/useChatMessages.retention.test.ts`; **badge rendering** by 6 cases in `ui/src/components/chat/view/subcomponents/MessageComponent.retention-badge.test.tsx` — rendering both EN and ZH corpora, asserting the text actually shown on screen (including thousand-separators like `1,204 tok`) and the policy/budget in the hover `title`, and that no badge appears on upstream boundaries, with one case running the full chain `compactMetadata → normalizedToChatMessages → component`. That file has been mutation-tested: forcing the badge render condition in `MessageComponent` to always-false makes 4 of the 6 positive cases fail while the 2 negative cases still pass — so it really tests rendering, not no-ops. The boundary is that jsdom is not a browser: styling, dark mode and real layout are outside the covered surface, so the §7 Web UI path is still worth running by hand on site.
- **UI `npx vitest run` is fully green: 118 files / 922 tests, 0 failed.** The 5 previously failing files each had a concrete cause and are fixed, all on the test side (no implementation file touched): `e2e/*.spec.mjs` are Playwright specs that vitest collected as unit tests (now excluded in `vite.config.js`); `server/routes/{commands,memory,uploads}.test.js` stand up a real Express server and hit it over a loopback socket, which overruns vitest's 5 s default under full-suite parallelism (now `testTimeout: 20000` — a genuinely hung route still fails, just later; no assertion was loosened); and 4 cases in `streamSmoother.test.ts` asserted the older implementation that `SmoothTextStream` replaced (rAF scheduling, word-boundary snapping, a live `averageCharsPerSecond`), now rewritten against the behaviour that actually ships and committed.
- A reproducible test-count claim: **root suite** (`pnpm test`, `tests/**/*.spec.ts` via `dist/`) measured 2026-09-12 at **520 = 518 passed / 0 failed / 0 cancelled / 2 skipped** (34.5 s, exit 0). The UI side is a *separate* vitest suite (922 cases, bullet above); the two are not summed — "520" in this text refers to the root suite only.
  - **Why CI's count is 10 lower than local**: upstream `.gitignore` treats `*.test.ts` as "local test drafts" (its comment: `Local test drafts (force-add intentional new tests with git add -f)`), and upstream keeps 4 such draft files (`tests/gateway/{upload-store,dialog-project-files,dialog-model-catalog,dialog-skills-permissions}.test.ts`, 10 cases total) out of the repo. We respect that convention and force-add only the tests we want as permanent regression guards. Local 520 − 10 = 510 is the CI count, and you can check it locally: `git check-ignore -v tests/gateway/upload-store.test.ts` prints a match on `.gitignore:200:*.test.ts`.
