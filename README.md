...
| | 上游（只做摘要） | 加了 Hippo |
|---|---|---|
| 压缩后上下文里，20 条早期事实**逐字**还在几条 | 0 | **6–18**（见下表） |
...
| 场景 | 上游 | Hippo | 胜负 |
|---|---:|---:|---|
| 英文 N=160 | 0 / 20 | **17.9 / 20** | 10 胜 0 平 0 负 |
| 中文 N=160 | 0 / 20 | **10.0 / 20** | 10 胜 0 平 0 负 |
| 英文 N=80 | 0 / 20 | **9.4 / 20** | 10 胜 0 平 0 负 |
| 中文 N=80 | 0 / 20 | **6.0 / 20** | 10 胜 0 平 0 负 |
...
corepack pnpm benchmark:demo        # 约 2 秒；英文 N=160：上游 0/20 → Hippo 18/20
...
- **默认权重换过，但不当成"优化带来的提升"。** 改权重是因为它对齐了调参结果且少一项；留出集上 rank 项的影响远小于噪声（7 胜 33 平 0 负，无显著）。真正的主效应是"逐字保留 vs 不做保留"。
