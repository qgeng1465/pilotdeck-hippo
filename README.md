...
- `ui/` 的 `tsc --noEmit` 退出码 **0**。fork 起点是 101 个文件 / 511 处报错，其中 501 处同一个根因：依赖图里同时存在两份 `@types/react`（`ui` 自己的 18.3.29 与根提升的 19.2.15），而 `lucide-react` 没有 `@types/react` peer，其 `.d.ts` 沿目录向上解析到了 19，于是每个图标都过不了 JSX 组件检查。修法是在 `ui/tsconfig.json` 用 `paths` 把 `react` / `react/*` 钉到 ui 自己那份类型（typecheck 专用，不改 `vite build` 的运行时别名，也不需要重装依赖）；剩下 10 处是真类型缺口，逐个按真实类型补上（`subagentState.isFailed`、`NormalizedMessage.userHint`、`RefObject<HTMLInputElement>` 等），没有用 `@ts-ignore` 或 `as any` 掩盖。`vite build` 退出码 0（40.4 s）。
- **保留徽章的测试边界（如实说明）**：网关侧「retention 随 agent_status 下发、上游路径完全不带该键」由根套件 `tests/context/retention-reporting.spec.ts` 覆盖；`compactMetadata → 徽章数据` 的映射（含畸形输入不抛异常）由 UI 侧 `ui/src/components/chat/hooks/useChatMessages.retention.test.ts` 14 例覆盖；**徽章的渲染**另由 `ui/src/components/chat/view/subcomponents/MessageComponent.retention-badge.test.tsx` 6 例覆盖——中英两个语料各自渲染、断言屏幕上真正显示的文字（含 `1,204 tok` 这类千分位）与悬停 `title` 里的策略和预算、上游边界不出现徽章，其中一例走完整链路 `compactMetadata → normalizedToChatMessages → 组件`。该文件做过**变异验证**：把 `MessageComponent` 的徽章渲染条件改成恒假后，6 例中 4 例正例失败、2 例反例照常通过，说明它确实在测渲染而不是在空转。边界是 jsdom 不是浏览器：样式、暗色模式与真实布局不在覆盖范围内，所以 §7 的 Web UI 路径仍值得现场亲手跑一遍。
- UI 侧 `npx vitest run` 全绿：**118 个文件 / 922 个用例，0 失败**。原先 5 个失败文件各有实因，均已修，且都是测试侧问题、没动实现：`e2e/*.spec.mjs` 是 Playwright 规格，被 vitest 当单测收集（已在 `vite.config.js` 排除）；`server/routes/{commands,memory,uploads}.test.js` 会起真的 Express 服务并走回环 socket，满载并行时会超过 vitest 默认的 5 s（已设 `testTimeout: 20000`——真正挂死的路由仍然失败，只是晚一些，没有放宽断言）；`streamSmoother.test.ts` 的 4 例断言的是被 `SmoothTextStream` 取代的旧实现（rAF 调度、词边界吸附、`averageCharsPerSecond` 动态更新），已按当前真正发布的行为重写并入库。
- 测试数量声明需可复核：**根套件**（`pnpm test`，`tests/**/*.spec.ts` 经 `dist/` 运行）2026-09-12 实测 **520 项 = 518 通过 / 0 失败 / 0 cancelled / 2 skipped**（34.5 s，退出码 0）。UI 侧是**独立**的 vitest 套件（922 例，见上一条），两者不合并计数——本文出现的 “520” 一律只指根套件。
  - **为什么 CI 的计数会比本地少 10**：上游 `.gitignore` 把 `*.test.ts` 当作“本地测试草稿”（原文注释：`Local test drafts (force-add intentional new tests with git add -f)`），上游自己留有 4 个草稿文件（`tests/gateway/{upload-store,dialog-project-files,dialog-model-catalog,dialog-skills-permissions}.test.ts`，合计 10 例）没有入库。我们尊重这条约定，只为确实要长期回归的测试做 `git add -f`。本地 520 − 10 = 510 即 CI 的计数，可直接复核：`git check-ignore -v tests/gateway/upload-store.test.ts` 会打印命中 `.gitignore:200:*.test.ts`。
