...
Context compaction is lossy. The upstream `CompactionEngine`, when context is tight, keeps the most recent span (the tail) **verbatim** and **summarizes** older history into a paragraph. Summaries are good at preserving "the gist" but drop low-frequency **precise facts** — gene names, frequency numbers, file paths, checkpoints, decision records. Yet those are exactly what an agent must reproduce verbatim on later tasks.

Hippo does not try to build "a better summarizer" — that only raises the quality ceiling within a lossy framework, and costs more. Hippo changes the mechanism: **part of the content never enters the lossy path at all.**
