...
export async function networkFetch(
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt += 1) {
    ...
    const timeout = options.timeoutMs && options.timeoutMs > 0
      ...
      : undefined;
    // Deliberately NOT unref'd: this timer is what guarantees the request
    // aborts on time. Unref'd, it never fires when the underlying transport
    // holds no libuv handle of its own (mocked/lightweight fetch impls), and
    // the process exits with the promise still pending. The timer is cleared
    // in finally() below, so it cannot outlive the request either way.
...
function delay(ms: number, signal?: AbortSignal): Promise<void> {
  // Not unref'd (see the timeout timer above): a pending retry delay is part
  // of the operation's progress, so it must be allowed to fire and continue
  // the retry instead of letting the process exit mid-backoff.
