...
function tokens(text) {
    ...
    for (const word of ascii) {
        if (!STOPWORD_BLOCKLIST.has(word))
            out.push(word);
    }
...
export function defaultEmbeddingModelPath() {
...
async function loadTransformersEmbedding(modelPath) {
    const specifier = "@huggingface/transformers";
    let module;
    try {
        module = await new Function(`return import(${JSON.stringify(specifier)})`)();
    }
    catch {
        return null;
    }
