...
function parseAgent(
  ...
  const compaction = parseAgentCompaction(rawAgent.compaction, diagnostics);
...
function parseAgentCompaction(
  value: unknown,
  diagnostics: PilotConfigDiagnostic[],
): PilotAgentConfig["compaction"] | undefined {
  if (value === undefined || value === null) return undefined;
  if (!isRecord(value)) {
    diagnostics.push({
      code: "CONFIG_AGENT_COMPACTION_INVALID",
      severity: "warning",
      message:
        "agent.compaction must be an object; ignoring it and using the default retention policy (\"hippo\").",
  ...
  if (retention !== "hippo" && retention !== "off") {
    diagnostics.push({
      code: "CONFIG_AGENT_COMPACTION_RETENTION_INVALID",
      severity: "warning",
      message: `agent.compaction.retention must be "hippo" or "off"; ignoring ${JSON.stringify(
        retention,
      )} and using the default ("hippo").`,
      path: "agent.compaction.retention",
      recoverable: true,
    });
    return {};
  }
