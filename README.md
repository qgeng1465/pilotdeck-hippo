...
| 已知边界（我们主动划的） | ① 收益是"对**当前任务**保真"，**不是**"记住被放弃的旧话题"——旧话题优势经诊断是 harness 句式共享造成的假象，[4.6](#46-话题切换旧话题还剩多少回答中途换话题)；② 换成**真摘要器**后，"当前话题答对率"的优势不成立（真摘要器自己也会把检查点抄进摘要，且同配置重复的摆动就有 3–4 题），[4.7](#47-真摘要器下的话题切换46-第一条边界的补充测量)；③ 改默认权重的理由是"对齐调参 argmax + 少一项"，**不是**"权重优化带来提升"（该对比留出集上不显著，[4.5](#45-留出集验证选过参数的-seed-一律不用)）。 |
...
1. **旧话题 A 那一列（shared 下的 2 与 5.9）是 harness 假象，decorrelated 下归零**。诊断证据：单目标消融里 `time only` 与 `rank only` 都是 **0/10**，`sim only` 单独就复现 5.0/10；存活事实在 A 段的位置均值 0.58（无"越近越留"倾向），进一步排除时近度；直接量 BM25，A 段事实对 B 请求是 **0.820** 而 A 段填充消息只有 **0.240**，把同样的事实改写成不含共享字段标签的散文后掉到 **0.370**——约 **78% 的优势来自共享句式**。把这条词面桥拆掉，A 列就是 **0/10**，与上游一致。**所以对外不要说"Hippo 保住了旧话题"，它没有。**
...
3. **两条诚实边界，都标出来而不是绕过去**：
   - 这个 harness 用固定文本 stub 摘要器，**摘要本身按构造不可能携带事实**，所以 `summary-only` 两臂都是 0，逐字列就是全部结论。这张表只回答"逐字保留块里有没有事实"，不回答"真摘要器下用户能不能答对问题"。**这条边界已经补上了**：`benchmark:real-llm-topic-switch`（§4.7）用真摘要器重做同一实验，结论是**这张表 B 列的 6/10 → 10/10 优势在真摘要器下不成立**——真摘要器自己不靠保留块就能把事实写进摘要，上游也因此能答对当前话题。请把 B 列读作"stub 摘要器口径下、逐字保留块与 stub 摘要的对比"，不要读成"用户能多答对多少"。
...
§4.6 用的是 stub 摘要器，因此它对"真摘要器下还能不能答对旧话题"没有发言权。本节用同一个话题切换转录稿、把摘要器换成真模型来补这个测量。要先把预期压低：**这是一次把我们的说法证伪的测量，不是一次加分**。脚本 `pnpm benchmark:real-llm-topic-switch`，出自 `benchmarks/results/real-llm-topic-switch-2026-09-11T12-29-27-528Z.json`。

协议：`N/topic=160`、每话题 10 条事实、每例抽 4 题问旧话题 A、4 题问当前话题 B；2 个留出集种子 × **2 次重复**（重复是本节的关键变量，下详）；摘要器与评审员均为 DeepSeek-V4-Flash（`max_tokens=8000`，16 次摘要**无一次截断**）。

| 事实措辞 | 变体 | 旧话题 A 答对 | A 重复内两次 | A：事实在**保留区内**（中位/10） | A：事实**只在摘要里**（中位/10） | 当前话题 B 答对 |
|---|---|---:|---|---:|---:|---:|
| shared | upstream | 11/16 | 4、7 | **0** | 4 | 16/16 |
| shared | Hippo | 16/16 | 8、8 | **6** | 2 | 16/16 |
| decorrelated | upstream | 16/16 | 8、8 | **0** | **8** | 16/16 |
| decorrelated | Hippo | 12/16 | 4、8 | **0** | 6 | 16/16 |

四点解读，按证据强度排序：

1. **真摘要器自己就会把旧话题带下去，"旧话题还答得出"不能算在本 fork 头上。** 两个 upstream 臂在旧话题上**保留区计数恒为 0**——上游没有保留块，而旧话题位于转录稿开头、远在保留尾部之外，所以这个 0 是实测而非推断。它们唯一的信息来源是摘要：`decorrelated` 下上游有 **8/10** 条 A 事实**只出现在摘要里**，答题 16/16 全对。现场若有人问"换话题后旧话题还在不在"，**先看保留区那一列**，不要拿答题正确率当记忆的证据。
2. **§4.6 的 shared 假象在真摘要器路径上原样重现**，两套 harness 因此互相印证而不是互相矛盾：fork 的 A 段保留区计数在 `shared` 下是 **6/10**，在 `decorrelated` 下归 **0**，上游两种措辞下都是 0。
3. **重复内方差 ≥ 臂间差异，本节不声明任何方向性结论。** 同一配置、同一种子集合，两次重复的 A 列答对数分别是 `shared/upstream` 4 与 7、`decorrelated/Hippo` 4 与 8——**同配置的摆动就有 3–4 题**。把两种措辞合起来看，旧话题总计 upstream 27/32、Hippo 28/32，差 1 题，远在摆动范围之内。当前话题 B 四臂全部 16/16（天花板），本就无法区分。
4. **那保留块在这里做了什么？** 可测的只有"逐字"这件事本身：`shared` 与 `decorrelated` 两种措辞下，fork 的 B 段保留区都是 8/10，上游是 6/10（尾部逐字 + 摘要转述）。即 fork 把当前话题的检查点**以确定性方式**多保住约 2 条逐字事实，而在这个已经到天花板的场景里它没有变成正确率差异。**这正是它的定位：可复现的确定性保留，不是平均正确率的提升**；正确率的提升要在 §4.3 那种摘要预算吃紧、摘要器必须丢东西的场景里才出现。

与 §4.3 一致的方差声明照旧：同模型既摘要又评审、每格样本小、网关在相同设置下会给出不同结果，因此本节只声明机制（保留区/摘要的来源分离）与否定性结论（旧话题记忆不归本 fork），不声明任何"提升 X 点"。
