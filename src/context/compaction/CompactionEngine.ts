...
/** Fraction of the tail token budget Hippo may spend on verbatim key messages. */
...
export class CompactionEngine {
  ...
  async run(input: CompactionInput): Promise<CompactionResult> {
    ...
    const scorePolicy = input.scorePolicy ?? this.options.scorePolicy;
    const compactPlan = await planFullCompactionMessages(
      planningMessages,
      tailTokenBudget,
      protectedToolNames,
      minTailMessages,
...
async function planFullCompactionMessages(
  messages: CanonicalMessage[],
  ...
  let messagesToSummarize: CanonicalMessage[] = [];
  ...
  if (scorePolicy && messagesToSummarize.length > 0) {
    ...
    if (retainedSet.size > 0) {
      messagesToSummarize = messagesToSummarize.filter((message) => !retainedSet.has(message));
    }
  }
