...
describe('processGrouping', () => {
  ...
  it('carries the retention summary off the folded boundary onto the process row', () => {
    // The boundary is filtered out of `detailMessages`, so the retention
    // summary has to survive on the synthetic summary or the row that stands in
    // for it has nothing to render.
    const retention = {
    ...
    const messages = [
      user('u1'),
      { ...compact('compact-ret', 200), compactRetention: retention },
      tool('bash-2', 'Bash', { command: 'deliver workbook' }, 300),
      assistant('a-final', 'Workbook delivered.', 400),
    ];
    ...
    expect(finalAssistant?.beforeProcessAttachments[0].processSummary.compactCount).toBe(1);
    expect(finalAssistant?.beforeProcessAttachments[0].processSummary.compactRetention).toEqual(retention);
  });

  it('reports no retention summary when the boundary carried none', () => {
    const messages = [
      user('u1'),
      compact('compact-plain', 200),
      assistant('a-final', 'Workbook delivered.', 400),
    ];
    ...
    expect(finalAssistant?.beforeProcessAttachments[0].processSummary.compactRetention).toBeUndefined();
  });
